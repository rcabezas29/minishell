...
/*   Updated: 2021/11/08 09:00:08 by rcabezas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	assign_arguments_from_list(t_list *tmp, t_exe *exe, int n)
{
	...
	args = malloc(sizeof(char *) * count_arguments(tmp));
	i = 0;
...
static void	fill_exe(t_cmd_info *cmd_info, t_exe *exe)
{
	t_list	*tmp;
	int		i;
	int		arg;
	...
	while (tmp)
	{
		...
		while (tmp && ((t_node *)tmp->content)->types != PIPE)
		{
			...
			{
				exe[i].cmd = ft_strdup(((t_node *)tmp->content)->prompts);
				arg = 1;
			}
			else if (((t_node *)tmp->content)->types == ARGUMENT && arg == 1)
				assign_arguments_from_list(tmp, exe, i);
			else if (((t_node *)tmp->content)->types == FILE_NAME)
				assign_fd_from_list(tmp, exe, i);
			tmp = tmp->next;
		}
		i++;
		if (tmp)
...
void	parser(t_cmd_info *cmd_info)
{
	...
	i = 0;
	exe = ft_calloc(sizeof(t_exe) * (cmd_info->no_pipes + 2), 1);
	fill_exe(cmd_info, exe);
}
